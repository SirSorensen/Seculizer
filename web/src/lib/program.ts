...
export class Program {
  ...
  constructor(json: ProgramAST, log: boolean = false) {
    ...
    if (json.knowledge) {
      json.knowledge.knowledge.forEach((knowledge: KnowledgeItem) => {
        knowledge.children.forEach((child: Type) => {
          this.init_participants.setKnowledgeOfParticipant(
            knowledge.id.value,
            child,
            false
          );
      ...
      if (this.log)
        console.log("Knowledge added to participants", this.init_participants);
    ...
    // Functions:
    if (json.functions) {
      json.functions.functions.forEach(
        (func: FunctionDefItem) => (this.functions[func.id.value] = func.params)
      );
    ...
    if (this.first == null)
      throw new Error(
        "Invalid json: no first frame created! First frame not properly initialized"
      );
  ...
  sendStmnt(stmnt: SendStatement, last: Frame) {
    if (stmnt.child.type == "messageSendStatement") {
      ...
      this.messageSendStmnt(
        stmnt.leftId.value,
        stmnt.rightId.value,
        messageSendStmnt.expressions,
        last
      );
    ...
    knowledge.forEach((expression) => {
      if (expression.child.type == "encryptExpression") {
        ...
        this.encryptExpr(
          senderId,
          receiverId,
          encryptedExpression.inner,
          encryptedExpression.outer,
          last,
          encrypted
        );
  ...
  encryptExpr(
    ...
    if (!encrypted) {
      ...
      let decryptable = last
        .getParticipants()
        .checkKeyKnowledge(receiverId, this.checkKeyRelation(outer));
