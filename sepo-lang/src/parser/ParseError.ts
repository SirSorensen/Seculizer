...
export const throwParseError = (errorMsg: IError, token: IToken, template: string) => {
  const error = new ParseError(`${errorMsg.refCode}: ${errorMsg.message(token)}`, token,
    { line: token.startLine ?? -1, column: token.startColumn ?? -1, start: token.startOffset,end: token.endOffset || -1, indent: getIndent(token) }
  );
  if (token) {
...
export const throwSimpleParseError = (errorMsg: string, token: IToken, template: string) => {
  const error = new ParseError(errorMsg, token, { line: token.startLine ?? -1, column: token.startColumn ?? -1, start: token.startOffset,end: token.endOffset || -1, indent: getIndent(token) });
